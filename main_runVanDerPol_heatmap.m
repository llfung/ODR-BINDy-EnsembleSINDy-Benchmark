...
%% common parameters, true VanDerPol system, signal power for noise calculation

% generate synthetic VanDerPol system data
ode_params.beta = 0.5;
x0 = [-2 1]';
n = length(x0); 
...
% get true VanDerPol system for comparison
true_nz_weights = zeros(10,2);
true_nz_weights(3,1)=1; % y(2)
true_nz_weights(2,2)=-1; % y(1)
true_nz_weights(3,2)=ode_params.beta ; %y(2)
true_nz_weights(8,2)=-ode_params.beta ; %y(1)^2*y(2)

% signal power for noise calculation
[~,x10]=ode45(@(t,x) vanderpol(t,x,ode_params),dt:dt:12,x0,options);
...
if runS
    ...
    for ieps = 1:length(epsL)
        ...
        for idt = 1:length(tEndL)
            ...
            [t,x]=ode45(@(t,x) vanderpol(t,x,ode_params),tspan,x0,options);
...
for ieps = 1:length(epsL)
    ...
    for idt = 1:length(tEndL)
        ...
        parfor ii = 1:nTest2
            ...
            %% Weak 
            [IMat,DMat] = weak(size(xobs,1),8,2,2,dt);
...
if saveTrue
    save('VanDerPol_simOutEFinal_weak');
end
